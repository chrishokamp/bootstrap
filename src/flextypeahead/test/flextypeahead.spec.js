describe('flextypeahead tests', function () {

  var $scope, $compile, $document, $timeout, $log;
  var changeInputValueTo, changeModelValueTo;

  beforeEach(module('ui.bootstrap.flextypeahead'));
  ...
  beforeEach(inject(function (_$rootScope_, _$compile_, _$document_, _$timeout_, $q, $filter, $sniffer) {
    $scope = _$rootScope_;
...
//    ];
    $scope.states = ['Alaska', 'California'];

    $scope.triggerFunc = function () {
      return $scope.typeaheadPrefix;
    };

    $scope.getLocation = function() {
      // TEST only
      var val = $scope.typeaheadPrefix;
    ...
    changeModelValueTo = function(modelName, value) {
  ...
  //utility functions
  var prepareEl = function (inputTpl) {
  ...
  var findTypeaheadElement = function (element) {
    return element.find('[flex-typeahead]');
  };
  ...
  describe('initial state and model changes', function () {
    ...
    it('should be closed by default', function () {
      var element = prepareEl('<div><div ng-model="result" flex-typeahead="item for item in source" typeahead-trigger-function="triggerFunc()" typeahead-loading="true"></div></div>');
      expect(element).toBeClosed();
    });

    it('should not get open on model change', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source"></div>');
  ...
  describe('basic functionality', function () {
    ...
    it('should open and close typeahead based on matches', function () {
      var element = prepareEl('<div><div ng-model="result" flex-typeahead="item for item in getLocation()" typeahead-trigger-function="triggerFunc()" typeahead-loading="true"></div></div>');

      var typeaheadEl = findTypeaheadElement(element);
      var ownsId = typeaheadEl.attr('aria-owns');

      expect(typeaheadEl.attr('aria-expanded')).toBe('false');
      expect(typeaheadEl.attr('aria-activedescendant')).toBeUndefined();

      changeModelValueTo('typeaheadPrefix', 'Ca');
      expect(element).toBeOpenWithActive(1, 0);

      expect(findDropDown(element).attr('id')).toBe(ownsId);
      expect(typeaheadEl.attr('aria-expanded')).toBe('true');
      var activeOptionId = ownsId + '-option-0';
      expect(typeaheadEl.attr('aria-activedescendant')).toBe(activeOptionId);
      expect(findDropDown(element).find('li.active').attr('id')).toBe(activeOptionId);

      changeModelValueTo('typeaheadPrefix', '');
      $scope.$digest();
      expect(element).toBeClosed();
      expect(typeaheadEl.attr('aria-expanded')).toBe('false');
      expect(typeaheadEl.attr('aria-activedescendant')).toBeUndefined();
    ...
    it('should allow expressions over multiple lines', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source \n' +
    ...
    it('should not open flex-typeahead if input value smaller than a defined threshold', function () {
      var element = prepareEl('<div><div ng-model="result" flex-typeahead="item for item in getLocation()" typeahead-min-length="2" typeahead-trigger-function="triggerFunc()" typeahead-loading="true"></div></div>');
//      changeModelValueTo('typeaheadPrefix', 'b');
      changeModelValueTo('typeaheadPrefix', 'b');
      expect(element).toBeClosed();
    });
    ...
    it('should support custom model selecting function', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="updaterFn(item) as item for item in source | filter:$viewValue"></div>');
    ...
    it('should support custom label rendering function', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item as formatterFn(item) for item in source | filter:$viewValue"></div>');
    ...
    it('should by default bind view value to model even if not part of matches', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue"></div>');
    ...
    it('should support the editable property to limit model bindings to matches only', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" typeahead-editable="false"></div>');
      changeInputValueTo(element, 'not in matches');
    ...
    it('should set validation errors for non-editable inputs', function () {
      ...
      var element = prepareEl(
        '<div><form name="form">' +
          '<input name="input" ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" typeahead-editable="false">' +
        '</form></div>');
    ...
    it('should bind loading indicator expression', inject(function ($timeout) {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in loadMatches()" typeahead-loading="isLoading"></div>');
      changeInputValueTo(element, 'foo');
    ...
    it('should support timeout before trying to match $viewValue', inject(function ($timeout) {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" typeahead-wait-ms="200"></div>');
      changeInputValueTo(element, 'foo');
      expect(element).toBeClosed();
    ...
    it('should cancel old timeouts when something is typed within waitTime', inject(function ($timeout) {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in loadMatches($viewValue) | filter:$viewValue" typeahead-wait-ms="200"></div>');
    ...
    it('should support custom templates for matched items', inject(function ($templateCache) {
      ...
      var element = prepareEl('<div><input ng-model="result" typeahead-template-url="custom.html" flex-typeahead="state as state.name for state in states | filter:$viewValue"></div>');
    ...
    it('should throw error on invalid expression', function () {
      var prepareInvalidDir = function () {
        prepareEl('<div><input ng-model="result" flex-typeahead="an invalid expression"></div>');
      };
  ...
  describe('selecting a match', function () {
    ...
    it('should invoke select callback on select', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" typeahead-on-select="onSelect($item, $model, $label)" flex-typeahead="state.code as state.name for state in states | filter:$viewValue"></div>');
    ...
    it('should correctly update inputs value on mapping where label is not derived from the model', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="state.code as state.name for state in states | filter:$viewValue"></div>');
      var inputEl = findInput(element);
  ...
  describe('pop-up interaction', function () {
    ...
    beforeEach(function () {
      element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue"></div>');
    });
  ...
  describe('promises', function () {
    ...
    beforeEach(inject(function ($q) {
      ...
      element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source()"></div>');
    }));
  ...
  describe('non-regressions tests', function () {
    ...
    it('issue 591 - initial formatting for un-selected match and complex label expression', function () {
      ...
      var inputEl = findInput(prepareEl('<div><input ng-model="result" flex-typeahead="state as state.name + \' \' + state.code for state in states | filter:$viewValue"></div>'));
      expect(inputEl.val()).toEqual('');
    });

    it('issue 786 - name of internal model should not conflict with scope model name', function () {
      $scope.state = $scope.states[0];
      var element = prepareEl('<div><input ng-model="state" flex-typeahead="state as state.name for state in states | filter:$viewValue"></div>');
      var inputEl = findInput(element);
    ...
    it('issue 863 - it should work correctly with input type="email"', function () {
      ...
      var element = prepareEl('<div><input type="email" ng-model="email" flex-typeahead="email for email in emails | filter:$viewValue"></div>');
      var inputEl = findInput(element);
    ...
    it('issue 964 - should not show popup with matches if an element is not focused', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in items($viewValue)"></div>');
      var inputEl = findInput(element);
    ...
    it('should properly update loading callback if an element is not focused', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" typeahead-loading="isLoading" flex-typeahead="item for item in items($viewValue)"></div>');
      var inputEl = findInput(element);
    ...
    it('issue 1140 - should properly update loading callback when deleting characters', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" typeahead-min-length="2" typeahead-loading="isLoading" flex-typeahead="item for item in items($viewValue)"></div>');
    ...
    it('should cancel old timeout when deleting characters', inject(function ($timeout) {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in loadMatches($viewValue) | filter:$viewValue" typeahead-min-length="2" typeahead-wait-ms="200"></div>');
      changeInputValueTo(element, 'match');
      changeInputValueTo(element, 'm');
    ...
    it('issue #1238 - allow names like "query" to be used inside "in" expressions ', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in query($viewValue)"></div>');
      changeInputValueTo(element, 'bar');
    ...
    it('issue #1773 - should not trigger an error when used with ng-focus', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" ng-focus="foo()"></div>');
      var inputEl = findInput(element);
  ...
  describe('input formatting', function () {
    ...
    it('should co-operate with existing formatters', function () {
      ...
      var element = prepareEl('<div><input ng-model="result.name" formatter flex-typeahead="state.name for state in states | filter:$viewValue"></div>'),
    ...
    it('should support a custom input formatting function', function () {
      ...
      var element = prepareEl('<div><input ng-model="result" typeahead-input-formatter="formatInput($model)" flex-typeahead="state as state.name for state in states | filter:$viewValue"></div>'),
      inputEl = findInput(element);
  ...
  describe('append to body', function () {
    it('append typeahead results to body', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" typeahead-append-to-body="true"></div>');
      changeInputValueTo(element, 'ba');
    ...
    it('should not append to body when value of the attribute is false', function () {
      var element = prepareEl('<div><input ng-model="result" flex-typeahead="item for item in source | filter:$viewValue" typeahead-append-to-body="false"></div>');
      changeInputValueTo(element, 'ba');
