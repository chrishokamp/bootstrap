  ...
  grunt.initConfig({
    ...
    concat: {
      dist: {
        ...
        dest: '<%= dist %>/app/scripts/lib/<%= filename %>-<%= pkg.version %>.js'
      },
      dist_tpls: {
        options: {
        ...
        src: [], //src filled in by build task
        dest: '<%= dist %>/app/scripts/lib/<%= filename %>-tpls-<%= pkg.version %>.js'
      }
    },
    copy: {
      demohtml: {
    ...
    uglify: {
      ...
      dist:{
        ...
        dest:'<%= dist %>/app/scripts/lib/<%= filename %>-<%= pkg.version %>.min.js'
      },
      dist_tpls:{
        src:['<%= concat.dist_tpls.dest %>'],
        dest:'<%= dist %>/app/scripts/lib/<%= filename %>-tpls-<%= pkg.version %>.min.js'
      }
    },
    html2js: {
      dist: {
